...
class SettingsFragment : PreferenceFragmentCompat() {
    ...
    private fun initializeThemeSettings() {
        val getCurrentTheme = MainApplication.preferences.getInt(
            "defaultTheme",
    ...
    private fun initializeStartupSettings() {
        ...
        startupToggle!!.onPreferenceChangeListener =
            Preference.OnPreferenceChangeListener { _: Preference?, newValue: Any ->
                val editor = MainApplication.preferences.edit()
...
URL: $url"""
    ...
    private fun showWebadminCredentialPopup(webadminFile: File, ini: Ini) {
        ...
        val dialog = MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(R.string.ok) { _: DialogInterface?, _: Int ->
                val newUsername =
                    layout.findViewById<EditText>(R.id.webadminUsername).text.toString()
                val newPassword =
                    layout.findViewById<EditText>(R.id.webadminPassword).text.toString()
    ...
    private fun initializeInstallSettings() {
        ...
        val abi = String.format(
            getString(R.string.settings_install_manually_abi),
...
$abi"""
    ...
    private val pickFileBinaryLauncher =
        registerForActivityResult<String, Uri>(ActivityResultContracts.GetContent()) { uri: Uri? ->
            InstallService.installLocal(
                requireActivity(),
                uri,
                InstallService.State.PICK_FILE_BINARY
            )
        }
    private val pickFileServerLauncher =
        registerForActivityResult<String, Uri>(ActivityResultContracts.GetContent()) { uri: Uri? ->
            InstallService.installLocal(
                requireActivity(),
                uri,
                InstallService.State.PICK_FILE_SERVER
            )
        }
...
${getString(R.string.ini4j_license_description)}"""
        ...
        version!!.setSummary(
            String.format(
                getString(R.string.settings_info_version),
